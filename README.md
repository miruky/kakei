...
- 一覧右上の検索欄でカテゴリ・メモ・金額を絞り込めます(空白区切りで複数語のAND)
- 右上の「データ」メニューからJSON/CSVで書き出し、JSON/CSVのどちらでも取り込めます。CSVは表計算ソフトでそのまま開けます
- 右上のアイコンでテーマを「端末に合わせる・ライト・ダーク」と切り替えます。設定は次回も引き継がれます
- キーボード操作: `←` `→`(または `[` `]`)で月送り、`n` で金額欄、`/` で検索欄へ
- 表示中の月はURL(`#2026-06`)に入るので、その月をブックマーク・共有できます
- ホーム画面に追加すればスタンドアロンのアプリとして開きます

口座連携・レシート読み取り・複数端末の自動同期はありません。引っ越しや共有は書き出したJSON/CSVを移すことで行います。

## アーキテクチャ

![kakeiのアーキテクチャ](docs/architecture.svg)

`ledger.ts` が台帳(検証・保存・入出力)、`stats.ts` が月次集計とカテゴリ内訳、`chart.ts` がSVG文字列の生成、`csv.ts` がCSVとの相互変換、`filter.ts` が一覧の絞り込み、`theme.ts` がテーマ選好の解決を担い、互いに独立した純粋なモジュールです。UIの `app.ts` は「表示中の月」「フォームが新規か編集か」「絞り込み語」だけを状態に持ちます。配色はCSSカスタムプロパティ参照で、`data-theme` 属性と `prefers-color-scheme` に追従します。テーマは描画前にインラインスクリプトで確定させ、リロード時のちらつきを防ぎます。`sw.js` はネットワーク優先・キャッシュフォールバックの素朴なService Workerです。
...
- `src/lib/ledger.ts` — 台帳。整数円での検証・CRUD・JSON入出力・一括取り込み
- `src/lib/stats.ts` — 月次集計・カテゴリ内訳・月キーの計算
- `src/lib/chart.ts` — 並列棒グラフとドーナツのSVG生成
- `src/lib/csv.ts` — 台帳とCSVの相互変換(RFC4180準拠のエスケープ)
- `src/lib/filter.ts` — カテゴリ・メモ・金額による一覧の絞り込み
- `src/lib/theme.ts` — テーマ選好の解決と保存
- `src/app.ts` — フォーム・月送り・カード・一覧・検索・テーマ・データ入出力のUI
